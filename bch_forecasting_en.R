...
# 7. Results Table ----
error_df <- bind_rows(results, .id = "Model")
print(error_df)
...
df_long <- df_pred %>% pivot_longer(cols = -ds, names_to = "Model", values_to = "Inflation")
...
g <- ggplot() +
  ...
  geom_line(data = df_long, aes(x = ds, y = Inflation, color = Model)) +
  labs(title = "Model Forecasts vs. Actual Inflation",
       subtitle = "Training until 2020, forecasts from test period",
       x = "Date", y = "Year-to-year Inflation (%)",
       caption = "Note: Colors and line styles distinguish actual vs. forecasted values.",
       color = "Model") +
